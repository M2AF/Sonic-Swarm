...
export default class RTCPeerConnection extends EventTarget {
  ...
  get localDescription () {
    // return this.#nullableDescription(this.#peerConnection.localDescription())
    const desc = this.#nullableDescription(this.#peerConnection.localDescription())
    if (!desc) return null
    // libdatachannel returns type 'offer' for answers when setLocalDescription()
    // no-arg was used. Use our tracked type instead.
    if (this.#localDescriptionType && desc.type !== this.#localDescriptionType) {
      return new RTCSessionDescription({ type: this.#localDescriptionType, sdp: desc.sdp })
    }
    return desc
  }
  ...
  async setLocalDescription (description) {
    ...
    if (description.type !== 'offer') {
      // libdatachannel's setLocalDescription() with explicit non-offer type
      // doesn't work correctly. Using no-arg lets it auto-determine the type,
      // but localDescription() then returns the wrong type ('offer' for answers).
      // Track the correct type ourselves to fix localDescription().
      this.#peerConnection.setLocalDescription()
      this.#localDescriptionType = description.type
      return
    }
  ...
  async setRemoteDescription (description) {
    ...
    // libdatachannel generates a=setup:actpass in answers, but per RFC 3264
    // answers must use "active" or "passive". Fix it here.
    let sdp = description.sdp
    if (description.type === 'answer' && sdp.includes('a=setup:actpass')) {
      sdp = sdp.replace('a=setup:actpass', 'a=setup:active')
    }

    this.#peerConnection.setRemoteDescription(sdp, description.type)
  }
}
